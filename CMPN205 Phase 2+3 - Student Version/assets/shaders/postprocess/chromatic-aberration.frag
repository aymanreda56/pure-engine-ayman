...
void main(){
    ...
    vec4 sampledTexture = texture(tex, tex_coord);
    vec4 left = texture(tex, vec2(tex_coord.x-STRENGTH, tex_coord.y));
    vec4 right = texture(tex, vec2(tex_coord.x+STRENGTH,tex_coord.y));

    frag_color = vec4(left.x, sampledTexture.y, right.z, sampledTexture.w);
}